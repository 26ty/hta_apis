...
func (e *entity) AccountList(input *model.Fields) (amount int64, output []*model.Customer_Account, err error) {
	db := e.db.Model(&model.Table{})

	err = db.Count(&amount).Joins("Account").
		Order("customer.created_time desc").
...
func (e *entity) GetByID(input *model.Field) (output *model.Table, err error) { //第一括號參數 第二括號回傳
	db := e.db.Model(&model.Table{}).Where("c_id = ?", input.CID)
	//first不會給空值
...
func (e *entity) GetByCIDAccount(input *model.Field) (output *model.Customer_Account, err error) {
	db := e.db.Model(&model.Table{})

	err = db.Joins("Account").
		Where("c_id = ?", input.CID).First(&output).Error

	return output, err
}

func (e *entity) Deleted(input *model.Table) (err error) {
	err = e.db.Model(&model.Table{}).Where("c_id = ?", input.CID).Delete(&input).Error //model = table的意思

	return err
}

func (e *entity) Updated(input *model.Table) (err error) {
	err = e.db.Model(&model.Table{}).Where("c_id = ?", input.CID).Save(&input).Error
