...
func (s *service) GetByIDCountersignUserListUser(input *model.Documents) (quantity int64, output []*model.CountersignUser_Account, err error) {
	amount, fields, err := s.Entity.GetByIDCountersignUserListUser(input)
	if err != nil {
		log.Error(err)
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{CuID: input.CuID})
	if err != nil {
		log.Error(err)
...
func (s *service) Updated_Bonita(ID string,ParentCaseID string) (err error) {
	field, err := s.Entity.GetByID(&model.Field{CuID: ID})
	if err != nil {
		log.Error(err)
	...
	input := model.Updated_Bonita{
		CuID : ID,
		BonitaParentCaseID : ParentCaseID,
	}
