...
func (s *service) AccountList(input *model.Fields) (quantity int64, output []*model.Customer_Account, err error) {
...
func (s *service) GetByCIDAccount(input *model.Field) (output *model.Customer_Account, err error) {
	field, err := s.Entity.GetByCIDAccount(input)
	if err != nil {
		log.Error(err)
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{CID: input.CID})
	if err != nil {
		log.Error(err)
