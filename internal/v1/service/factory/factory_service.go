...
func (s *service) FLMListUser(input *model.Fields) (quantity int64, output []*model.Base, err error) {
	amount, fields, err := s.Entity.FLMListUser(input)
	if err != nil {
		log.Error(err)
...
func (s *service) GetByFIDFLMListUser(input *model.Field) (output *model.Base, err error) {
	field, err := s.Entity.GetByFIDFLMListUser(input)
	if err != nil {
		log.Error(err)
