...
func (s *service) MachineCombinedListLast(input *model.Fields) (quantity int64, output []*model.Machine_Combined_Last, err error) {
	amount, fields, err := s.Entity.MachineCombinedListLast(input)
	if err != nil {
		log.Error(err)
...
func (s *service) GetByPIDMachineCombinedListLast(input *model.Fields) (quantity int64, output []*model.Machine_Combined_Last, err error) {
	amount, fields, err := s.Entity.GetByPIDMachineCombinedListLast(input)
	if err != nil {
		log.Error(err)
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{McID: input.McID})
	if err != nil {
		log.Error(err)
