...
func (s *service) Created(input *model.Created) (output *model.Base, err error) {
	...
	output.SID = util.GenerateUUID()
	output.CreatedTime = util.NowToUTC()
	//output.Date = util.ChangeToDate()
	//time.Format("2019-07-10",input.Date.String())
...
func (s *service) AccountList(input *model.Fields) (quantity int64, output []*model.SalesCallRecord_Account, err error) {
	amount, fields, err := s.Entity.AccountList(input)
	if err != nil {
		log.Error(err)
...
func (s *service) GetBySIDAccount(input *model.Field) (output *model.SalesCallRecord_Account, err error) {
	field, err := s.Entity.GetBySIDAccount(input)
	if err != nil {
		log.Error(err)
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{SID: input.SID})
	if err != nil {
		log.Error(err)
