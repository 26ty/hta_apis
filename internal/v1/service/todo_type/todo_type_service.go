...
func (s *service) GetByUserID(input *model.Field) (output []*model.Base, err error) {
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{TtID: input.TtID})
	if err != nil {
		log.Error(err)
		return err
	}
