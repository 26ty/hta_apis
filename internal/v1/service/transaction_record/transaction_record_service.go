...
func (s *service) GetByDocumentIDUserList(input *model.Fields) (quantity int64, output []*model.Record_user_list, err error) {
	amount, Fields, err := s.Entity.GetByDocumentIDUserList(input)
	if err != nil {
		log.Error(err)
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{TrID: input.TrID})
	if err != nil {
		log.Error(err)
